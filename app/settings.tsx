...
export default function SettingsScreen() {
  ...
  // Developer options — tap "DEVELOPER OPTIONS" label 5 times to unlock
      ...
      >
        ...
        >
          <Text style={styles.devLabel}>
            {devUnlocked ? '🔓 DEVELOPER OPTIONS' : 'DEVELOPER OPTIONS'}
          </Text>
          {!devUnlocked && devTaps > 0 && (
            <Text style={styles.devHint}>{5 - devTaps} more taps...</Text>
          )}
        </Pressable>
        ...
        {devUnlocked && (
          <OffsetCard style={[styles.section, styles.devSection]}>
            <Text style={styles.devInfo}>
              {'Storage: @dog_tracker_data\nThis will wipe all data and restart onboarding.'}
            </Text>
            <Pressable
              onPress={handleReset}
...
const styles = StyleSheet.create({
  ...
  activityLabel: { fontFamily: PIXEL_FONT, fontSize: 7, color: Colors.textMuted, flex: 1 },
